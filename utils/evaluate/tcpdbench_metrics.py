...
Copyright (c) 2020 - The Alan Turing Instit


Notes:

* the true value inputs are a dictionary, where the keys correspond to the 
annotator ids.
* the predictions are a list of predicted CPs (no annotator ids)
* in our case, since we already have aggregated - there is only a need to make 
the predictions a dictionary with a single key: 0.
"""
...
def true_positives(T, X, margin=5):
    ...
    set()
    """
    margin = np.timedelta64(margin, "D")  # added, so that it handles days
...
def f_measure(annotations, predictions, margin=5, alpha=0.5, return_PR=False):
    ...
    # as page 6: "we include t = 1 as a trivial change point in Tk and X."
    Tks = {k + 1: set(annotations[uid]) for k, uid in enumerate(annotations)}
    for Tk in Tks.values():
        # Tk.add(0)
        Tk.add(np.datetime64(0, "D"))  # '1970-01-01'

    X = set(predictions)
    # X.add(0)
    X.add(np.datetime64(0, "D"))  # '1970-01-01'. Trivial CP.

    # Create a set containing the true CPs
    Tstar = set()
    for Tk in Tks.values():
        for tau in Tk:
            Tstar.add(tau)
